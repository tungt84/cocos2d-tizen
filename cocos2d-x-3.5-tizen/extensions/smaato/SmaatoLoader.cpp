...
NS_CC_BEGIN
    ...
    bool SmaatoLoader::init()
    {
        ...
        scheduler->schedule(schedule_selector(SmaatoLoader::update), this, 0, false);
#endif
        return true;
    }
            ...
            std::vector<char*>* beacons)
    {
        ...
        if (file_char[0] == 'G' && file_char[1] == 'I' && file_char[2] == 'F') { //gif file
...
#endif
#if USING_COCOS2D_VERSION == COCOS2D_VERSION_3X
            const char* fullFilename = FileUtils::getInstance()->getWritablePath().append("banner.gif").c_str();
#endif
            ...
            if (image->initWithImageData((const unsigned char *) file_char, buffer->size())) {
